document.addEventListener("DOMContentLoaded", async () => {
  ...
  // Note: The 'active' class toggling is handled by audioAPI.js
  // canvas.js only reads the active state for visual effects
  ...
  function drawWaves() {
    ...
    for (let i = 0; i < waveColors.length; i++) {
      ...
      for (let x = 1; x < canvas.width; x++) {
        let y =
          yOffsets[i] +
          Math.sin(x * frequencies[i] + waveOffset + i * 2) * amplitudes[i];
        ctx.lineTo(x, y);
      }
  ...
  function animate() {
    ...
    const rainActive = document
      .querySelector('[data-sound="rain"]')
      .classList.contains("active");
    const fireActive = document
      .querySelector('[data-sound="fire"]')
      .classList.contains("active");
    const thunderActive = document
      .querySelector('[data-sound="thunder"]')
      .classList.contains("active");
    const nightActive = document
      .querySelector('[data-sound="night"]')
      .classList.contains("active");
    const wavesActive = document
      .querySelector('[data-sound="waves"]')
      .classList.contains("active");
